...
import './sign-up-interest-desired.css'; // Reusing the same CSS file for styling

const SignUpInterestDesired = () => {
  const navigate = useNavigate();
  const [newDesireID, setNewDesireID] = useState(null);
  const [cookies, setCookie] = useCookies(['desire', 'desireInterest']);
  const [formData, setFormData] = useState({ DesireID: 0, InterestID: 0 });
  const [selectedValues, setSelectedValues] = useState([]);

  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('newDesireID='));
    if (cookieValue) {
      const newdesireID = cookieValue.split('=')[1];
      setNewDesireID(newdesireID);
      setFormData(prevState => ({ ...prevState, DesireID: newdesireID }));
    }
  }, []);
  ...
  const handleFormSubmit = async event => {
    ...
    for (const selectedOption of selectedValues) {
      ...
      const response = await api.post('/DesiredInterests/', formDataForOption);
      const newDesiredInterest = response.data;
      const newDesiredInterestID = newDesiredInterest.DesiredInterestID;
      console.log(newDesiredInterestID);
      setCookie('newDesiredInterestID', newDesiredInterestID, { path: '/' });
    }
    setFormData({ DesireID: newDesireID, InterestID: 0 });
    navigate('/home');
  };
...
<div className='wrapper'>
  <div className="form-container">
    <h2>Select your desired interests</h2>
...
export default SignUpInterestDesired;
